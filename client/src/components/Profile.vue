                ...
                <div>
                    <ul>
                        ...
                        <li v-if="profileData.segments[0].stats.season6Wins">
                            <h4>Season 6 Wins</h4>
                            <p>{{profileData.segments[0].stats.season6Wins.displayValue}}</p>
...
export default {
    ...
    async created() {
        ...
        try {
            const response = await axios.get(`http://localhost:5000/api/v1/profile/${this.$route.params.platform}/${this.$route.params.gamertag}`);
